...
from .models import KP, Battledetail
...
def pokeinput():
    ...
    count = 0
    for i in range(400):
        img = cv2.imread("/home/wagou/djangoconf/images/"+(str)(i+1) + ".png",0)
        if not img is None:
            temp.append(img)
            temp2.append(count)
            if i == 181:
                ...
                count += 1
        for j in range(5):
            img = cv2.imread("/home/wagou/djangoconf/images/"+(str)(i+1) + "_" + (str)(j+1) + ".png",0)
            if not img is None:
                temp.append(img)
                temp2.append(count)
                if i == 371:
...
def getpoke(impimg,key,mode):
    # データ読み込み
    try:
        with open(BASE_DIR / 'static/data/PokeDataSimple.csv', encoding="shift-jis") as f:
            reader = csv.reader(f)
        ...
        cv2.imwrite(str(BASE_DIR) + "/static/img/" + dt_now.strftime('%Y%m%d_%H%M%S')+".png", resultimg)
